...
impl<T: NonPrim> Int for T {
    const SIGNED: bool = <T as NonPrim>::SIGNED;
    const BITS: u32 = <T as NonPrim>::BITS;
    const ZERO: Self = <T as NonPrim>::ZERO;
    const ONE: Self = <T as NonPrim>::ONE;
    const MIN: Self = <T as NonPrim>::MIN;
    const MAX: Self = <T as NonPrim>::MAX;
    type Signed = <T as NonPrim>::Signed;
    type Width = <T as NonPrim>::Width;
}
